...
const PRODUCTS = [
  {
    id: 'collar1',
    name: 'Leather Breakaway Collar',
    category: 'leather',
    type: 'Safety Collar',
    price: 19.99,
    desc: 'Genuine leather with a breakaway buckle, available in multiple colours.',
    image: '/images/accessories/collar1.png',
  },
  {
    id: 'collar2',
    ...
    type: 'Outdoor Collar',
    price: 12.99,
    desc: 'High‑visibility reflective stitching, ideal for evening adventures.',
    image: '/images/accessories/collar2.png',
  },
  {
    id: 'collar3',
    name: 'Bow Tie Collar Set',
    category: 'fancy',
    type: 'Fashion Collar',
    price: 14.99,
    desc: 'Removable bow tie, perfect for special occasions.',
    image: '/images/accessories/collar3.png',
  },
  {
    id: 'collar4',
    ...
    type: 'Leash',
    price: 24.99,
    desc: 'Ergonomic handle, 16‑ft tape leash for safe outdoor walks.',
    image: '/images/accessories/collar4.jpg',
  },
];
    ...
    <div className="font-sans bg-[#faf8f6] text-[#0f2a2f] min-h-screen">
      <section className="relative bg-gradient-to-br from-[#0f2a2f] to-[#2a6b6b] text-white py-20 text-center">
        <span className="inline-block bg-white/20 backdrop-blur-md px-5 py-1.5 rounded-full text-xs font-semibold tracking-widest uppercase mb-6 border border-white/20">
          <i className="fas fa-band-aid"></i> Walk in Style
        </span>
        <h1 className="font-['Playfair_Display'] text-4xl md:text-6xl font-bold mb-4">Collars & Leashes</h1>
        <p className="text-lg opacity-90 max-w-xl mx-auto">Safe, stylish, and comfortable gear for every outing.</p>
      </section>
          ...
          {filtered.map(product => (
            <div key={product.id} className="bg-white rounded-2xl overflow-hidden shadow-lg flex flex-col transition-transform duration-300 hover:-translate-y-1.5 hover:shadow-2xl">
              <div className="relative h-56 cursor-pointer" onClick={() => openZoom(product.image)}>
                <img src={product.image} alt={product.name} className="w-full h-full object-cover" onError={(e) => { e.target.src = 'https://via.placeholder.com/400x300?text=Collar'; }} />
