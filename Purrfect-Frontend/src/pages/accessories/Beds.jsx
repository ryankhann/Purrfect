...
const PRODUCTS = [
  {
    ...
    type: 'Enclosed Bed',
    price: 45.99,
    desc: 'Soft faux‑fur interior with a secure hideaway design.',
    image: '/images/accessories/bed-1.jpg',
  },
  {
    id: 'bed2',
    name: 'Tree House Perch',
    category: 'tree',
    type: 'Cat Tree',
    price: 129.99,
    desc: 'Multi‑level tower with sisal scratching posts.',
    image: '/images/accessories/tree.png',
  },
  {
    id: 'bed3',
    ...
    type: 'Hammock Bed',
    price: 22.99,
    desc: 'Clip‑on radiator bed – perfect for sun‑loving cats.',
    image: '/images/accessories/hammock.png',
  },
  {
    id: 'bed4',
    name: 'Donut Cuddler',
    ...
    type: 'Donut Bed',
    price: 34.99,
    desc: 'Ultra‑plush self‑warming bed for deep sleep.',
    image: '/images/accessories/donut.png',
  },
];

const FILTERS = [
  { key: 'all', label: 'All Beds' },
  { key: 'cave', label: 'Cave Beds' },
  { key: 'tree', label: 'Cat Trees' },
  { key: 'hammock', label: 'Hammocks' },
  { key: 'donut', label: 'Donut Beds' },
];
    ...
    <div className="font-sans bg-[#faf8f6] text-[#0f2a2f] min-h-screen">
      <section className="relative bg-gradient-to-br from-[#0f2a2f] to-[#2a6b6b] text-white py-20 text-center">
        <span className="inline-block bg-white/20 backdrop-blur-md px-5 py-1.5 rounded-full text-xs font-semibold tracking-widest uppercase mb-6 border border-white/20">
          <i className="fas fa-bed"></i> Comfort Zone
        </span>
        <h1 className="font-['Playfair_Display'] text-4xl md:text-6xl font-bold mb-4">Beds & Furniture</h1>
        <p className="text-lg opacity-90 max-w-xl mx-auto">Give your cat the perfect place to rest and play.</p>
      </section>
          ...
          {filtered.map(product => (
            <div key={product.id} className="bg-white rounded-2xl overflow-hidden shadow-lg flex flex-col transition-transform duration-300 hover:-translate-y-1.5 hover:shadow-2xl">
              <div className="relative h-56 cursor-pointer" onClick={() => openZoom(product.image)}>
                <img src={product.image} alt={product.name} className="w-full h-full object-cover" onError={(e) => { e.target.src = 'https://via.placeholder.com/400x300?text=Bed'; }} />
