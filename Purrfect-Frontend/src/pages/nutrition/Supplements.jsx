...
const PRODUCTS = [
  {
    ...
    price: 18.99,
    desc: 'Rich in omega‑3 for a glossy coat and healthy skin.',
  ...
  },
  {
    ...
    price: 22.99,
    desc: 'Supports gut health and digestion.',
  ...
  },
  {
    ...
    category: 'chew',
    type: 'Soft Chew Supplement',
    price: 25.99,
    desc: 'Glucosamine & chondroitin for joint support.',
    image: '/images/food/chews.png',   // replace with real image
  },
  {
    id: 'supp4',
    name: 'Multivitamin Tabs',
    category: 'tablet',
    type: 'Tablet Supplement',
    price: 15.99,
    desc: 'Daily multivitamin for overall vitality.',
    image: '/images/food/multivitamin.png',
  },
];
  ...
  return (
    <div className="font-sans bg-[#faf8f6] text-[#0f2a2f] min-h-screen">
      ...
      <section className="relative bg-gradient-to-br from-[#0f2a2f] to-[#2a6b6b] text-white py-20 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <span className="inline-block bg-white/20 backdrop-blur-md px-5 py-1.5 rounded-full text-xs font-semibold tracking-widest uppercase mb-6 border border-white/20">
            <i className="fas fa-capsules"></i> Health & Wellness
          </span>
          <h1 className="font-['Playfair_Display'] text-4xl md:text-6xl font-bold mb-4">Cat Supplements</h1>
          <p className="text-lg opacity-90 max-w-xl mx-auto">Boost your cat’s health with our vet‑formulated supplements.</p>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 my-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProducts.map(product => (
            <div key={product.id} className="food-card bg-white rounded-2xl overflow-hidden shadow-lg flex flex-col transition-transform duration-300 hover:-translate-y-1.5 hover:shadow-2xl">
              <div className="relative h-56 cursor-pointer" onClick={() => openZoom(product.image)}>
                <img src={product.image} alt={product.name} className="w-full h-full object-cover" onError={(e) => { e.target.src = 'https://via.placeholder.com/400x300?text=Supplement'; }} />
      ...
      <div className="text-center text-sm text-gray-400 py-8 border-t border-gray-200 mt-10">
        <p>All supplements are manufactured in FDA‑registered facilities. 🐾</p>
      </div>
