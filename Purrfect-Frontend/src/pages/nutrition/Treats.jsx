...
const PRODUCTS = [
  ...
  },
  {
    ...
    price: 9.99,
    desc: 'Perfect for training – tiny, tasty, healthy.',
    image: '/images/food/chicken-liver.png',
  },
  {
    id: 'treat4',
    name: 'Salmon Crunchy Strips',
    category: 'crunchy',
    type: 'Crunchy Treats',
    price: 11.49,
    desc: 'Grain‑free, baked to perfection.',
    image: '/images/food/SalmonCrunch.jpg',
  },
];
  ...
  return (
    <div className="font-sans bg-[#faf8f6] text-[#0f2a2f] min-h-screen">
      <section className="relative bg-gradient-to-br from-[#0f2a2f] to-[#2a6b6b] text-white py-20 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          <span className="inline-block bg-white/20 backdrop-blur-md px-5 py-1.5 rounded-full text-xs font-semibold tracking-widest uppercase mb-6 border border-white/20">
            <i className="fas fa-candy-cane"></i> Delicious Rewards
          </span>
          <h1 className="font-['Playfair_Display'] text-4xl md:text-6xl font-bold mb-4">Cat Treats</h1>
          <p className="text-lg opacity-90 max-w-xl mx-auto">Irresistible snacks for training, bonding, or just because.</p>
        </div>
      </section>
          ...
          {filteredProducts.map(product => (
            <div key={product.id} className="food-card bg-white rounded-2xl overflow-hidden shadow-lg flex flex-col transition-transform duration-300 hover:-translate-y-1.5 hover:shadow-2xl">
              <div className="relative h-56 cursor-pointer" onClick={() => openZoom(product.image)}>
                <img src={product.image} alt={product.name} className="w-full h-full object-cover" onError={(e) => { e.target.src = 'https://via.placeholder.com/400x300?text=Treat'; }} />
      ...
      <div className="text-center text-sm text-gray-400 py-8 border-t border-gray-200 mt-10">
        <p>All treats are made with natural ingredients and no artificial colors. 🐾</p>
      </div>
